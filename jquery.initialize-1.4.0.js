...
(function ($) {
  ...
  $.fn.initialize = function (options) {
    ...
    var setEvents = function () {
      ...
      var subElements = options.elements;
      for (var countForSubElsObj = 0, len = getSize(subElements); countForSubElsObj < len; countForSubElsObj++) {
        for (var countForSubElsEventsObj = 0, len2 = getSize(subElements[countForSubElsObj].events); countForSubElsEventsObj < len2; countForSubElsEventsObj++) {
          $(self).on(subElements[countForSubElsObj].events[countForSubElsEventsObj].name, subElements[countForSubElsObj].name, subElements[countForSubElsObj].events[countForSubElsEventsObj].fn);
        }
      }
    }
